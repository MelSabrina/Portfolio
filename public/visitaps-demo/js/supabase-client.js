...
if (window.MOCK_MODE) {
  window.VisitAuth = {
    ...
    getSession: function () {
      var role = sessionStorage.getItem('mock_user_role');
      var session = role ? { user: { id: window.MOCK_USERS[role].auth_uid } } : null;
      return Promise.resolve({ session: session, error: null });
    },
    getUser: function () {
      var role = sessionStorage.getItem('mock_user_role');
      var user = role ? { id: window.MOCK_USERS[role].auth_uid } : null;
      return Promise.resolve({ user: user, error: null });
    },
    isLoggedIn: function () {
      return Promise.resolve(!!sessionStorage.getItem('mock_user_role'));
    },
